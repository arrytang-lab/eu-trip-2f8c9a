...
const CACHE = 'eu2026-v10';
const ASSETS = ['./', './index.html', './apple-touch-icon.png', './manifest.webmanifest'];

self.addEventListener('install', e => {
  e.waitUntil((async () => {
    const c = await caches.open(CACHE);
    // cache:'reload' 绕过浏览器自身 HTTP 缓存，确保存下的是当前最新版本（否则离线副本会落后一版）
    await Promise.all(ASSETS.map(async u => {
      try { await c.put(u, await fetch(new Request(u, { cache: 'reload' }))); } catch (err) {}
    }));
...
self.addEventListener('fetch', e => {
  ...
  e.respondWith((async () => {
    ...
    try {
      // no-cache = 每次向服务器回源校验（内容没变则走 304，几乎不花流量），保证改版后立刻生效
